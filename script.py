...
from user_order import USER_IDS_BY_SIZE_DESC
from utils import (
    catch_exceptions,
...
if __name__ == "__main__":
    ...
    # sort_jsonl() re-sorts the final result file by user id.
    _lpt_rank = {u: i for i, u in enumerate(USER_IDS_BY_SIZE_DESC)}
    unprocessed_users.sort(key=lambda u: _lpt_rank.get(u, len(USER_IDS_BY_SIZE_DESC)))
    ...
    if _srsb_t0 is not None:
        print(f"__MAKESPAN__ {time.perf_counter() - _srsb_t0:.6f}", file=sys.stderr, flush=True)
